...
public class PentominoBuilder {
    ...
    public static void makeDatabase()
    {
        ...
        for(int i=0;i<basicDatabase.length;i++)
        {
            ...
            for(int j=0;j<tempDatabase3.length;j++)
            {
                tempDatabase3[j]=tempDatabase[j+tempDatabase.length/2];
            }
            //add the found pieces of just 1 basic piece to the database
            database.add(tempDatabase2);
            database.add(tempDatabase3);
            }
            else {
    ...
    /**
     * inverse the matrix horizontally
     * it makes a copy, the input matrix stays unchanged
     * @param data the matrix
     * @return the inverted matrix
     */
